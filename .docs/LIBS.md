...
| `lib-strat-range` | 1 | **`2`** | 2026-06-08 | — | publié. Setups d'une stratégie de RANGE (cassure/réintégration) sur un range quelconque (OR, Asian, London…). Exports : `swingStop(length, tolPct, dir)`, UDT `RangeSetup`, `detectRangeSetup(...)` (machine d'état d'excursion + géométrie SL/TP, range-agnostique, à appeler chaque barre hors conditionnel). **v2** : ajoute `entryTrigger(prevLevel, dir, breakOnClose) → [fired, price]` (trigger d'entrée bulle→flèche). Pur : aucun `input.*`, aucun dessin. Consommé par `strat-OR.pine`. |
...
| `lib-risk` | 1 | **`1`** | 2026-06-08 | — | publié. Gestion du RISQUE/exposition, générique à toute strat. Enums `BeMode`/`DirMode` ; `riskSize(equity, entry, sl, pointvalue, riskPct, maxLev)` (sizing par risque + cap levier) ; `directionAllowed(dirMode, netDir) → [allowLong, allowShort]` (filtre hedge/net) ; `positionEvents(dir, entry, tp1, tp3, stopNow, runnerPct, beMode, earlyBEPct, slGreyed, runner) → [stopHit, tp3Hit, beArm, runnerBirth]`. Pur, aucun `strategy.*`. Consommé par `strat-OR.pine`. |
...
import mpilard/lib_strat_range/2 as stratRange
import mpilard/lib_strat_draw/1  as stratDraw
import mpilard/lib_risk/1        as risk
```
