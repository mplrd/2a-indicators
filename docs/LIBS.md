...
| `lib-ma` | 1 | **`3`** | 2026-05-29 | `lib-bollinger` v3, `lib-series` v2 | publié (v2 ajoute `projectSMA()` ; **v3** : ajoute `dema(src, length)` (Double EMA, Mulloy 1994, variante "zero-lag" — building block de `lib_macd.zeroRetard()`) ; bump du dep lib_bollinger /1 → /3 pour consistency, signature `ma.ribbon` inchangée) |
...
| `lib-macd` | 1 | **`1`** | 2026-05-29 | `lib-ma` v3 | publié. **v1** : oscillateur MACD et variantes. Expose `zeroRetard(src, fastLen, slowLen, signalLen) → [macd, signal, hist, average]` — variante "Zero Retard" portée d'un indicateur de référence validé : DEMA pour fast/slow ET pour la signal line (réduit le lag MACD EMA-based), `average` = SMA(MACD, signalLen) comme référence lente. Consommé par `macd-zr.pine`. |
