...
| `lib-stochrsi` | 1 | **`1`** | 2026-05-29 | — | publié. **v1** : `compute(src, stochLen, rsiLen, smoothK, smoothD, log, useAvg) → [k, d]` — Stochastic RSI (oscillateur stochastique appliqué au RSI), portage de l'implémentation v5 validée. Pipeline : log(src) optionnel → RSI → Stoch → smooth K → smooth D, option `useAvg` qui remplace K par avg(K, D) pour lisser plus. Consommé par `divergences.pine`. |
