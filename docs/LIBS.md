...
| `lib-ma` | 1 | **`3`** | 2026-05-29 | `lib-bollinger` v3, `lib-series` v2 | publié (v2 ajoute `projectSMA()` ; **v3** : ajoute la trilogie "zero-lag" — `dema()` (Mulloy '94, 2 passes), `tema()` (Mulloy '94, 3 passes), `zlema()` (Ehlers '94, lag-shifted input) — + enum `MaMode { DEMA, TEMA, ZLEMA }` + dispatcher générique `apply(mode, src, length)` qui retourne la MA correspondante. Permet aux indicateurs paramétrables (`lib_macd.zeroLag`, futurs) de prendre un `simple MaMode` en input et déléguer le choix sans switch dupliqué. Bump du dep lib_bollinger /1 → /3 pour consistency, signature `ma.ribbon` inchangée) |
...
| `lib-macd` | 1 | **`1`** | 2026-05-29 | `lib-ma` v3 | publié. **v1** : oscillateur MACD et variantes "zero-lag". Expose `zeroLag(src, fastLen, slowLen, signalLen, mode) → [macd, signal, hist, average]` — variante paramétrable par `ma.MaMode` (DEMA / TEMA / ZLEMA), portée d'un indicateur de référence validé (v5 = DEMA only) avec extension du choix. Fast/slow ET signal line utilisent la MA choisie ; `average` = SMA(MACD, signalLen) comme référence lente non-zero-lag. Consommé par `macd-zr.pine`. |
